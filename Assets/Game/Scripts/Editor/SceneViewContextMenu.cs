...
sealed class SceneViewContextMenu : Attribute {
    ...
    public SceneViewContextMenu(string path) {
        this.path = path;
    }
}
...
public class SceneViewContextMenuInitializer : Editor {
    ...
    [MenuItem("Window/SceneView Context Menu/Enable", true)]
    static bool EnableTest() {
        return !IsRegistered();
    }

    [MenuItem("Window/SceneView Context Menu/Disable", true)]
    static bool DisableTest() {
        return IsRegistered();
    }
    ...
    static void OnSceneGUI(SceneView sceneview) {
        ...
        if (Event.current.modifiers == modifier && Event.current.button == 1 &&
    ...
    static void AddDefaultMenus(GenericMenu menu, Ray ray) {
        ...
        if (Selection.gameObjects.Length > 0) {
            var head = "Selection (" + (Selection.gameObjects.Length == 1
                           ? Selection.gameObjects[0].name
                           : Selection.gameObjects.Length + " GameObjects") + ")";
            ...
            menu.AddItem(new GUIContent(head + "/Reset Position"), false, () => ForEach(Selection.gameObjects, x => {
                if (x.transform.parent != null) x.transform.localPosition = Vector3.zero;
            }));
            menu.AddItem(new GUIContent(head + "/Reset Rotation"), false, () => ForEach(Selection.gameObjects, x => {
                if (x.transform.parent != null) x.transform.localRotation = Quaternion.identity;
            }));
            menu.AddItem(new GUIContent(head + "/Reset Scale"), false, () => ForEach(Selection.gameObjects, x => {
                if (x.transform.parent != null) x.transform.localScale = Vector3.one;
            }));
        }
...
public class ContextMenuBuilder {
    ...
    public ContextMenuBuilder Item(string label, Action<object> callBack, object parameter, bool disabled = false,
        bool selected = false) {
...
public struct ContextItem {
    ...
    public ContextItem(string label, Action<object> callBack, object callBackData, bool disabled = false,
        bool selected = false) {
        this.label = label;
        this.disabled = disabled;
        this.selected = selected;
